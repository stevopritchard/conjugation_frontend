...
class Reference extends React.Component {
  ...
  async listFavourites() {
    ...
    try {
      await Promise.all(
        this.props.favourites.map(async function (favourite) {
          const response = await fetch(
            'https://rocky-citadel-06291.herokuapp.com/favourite_verbs',
            {
              method: 'post',
    ...
    return (
      <div>
        ...
        {verbSelected === true ? ( //if false, sets 'initial state' value
          ...
          <Scroll>
            <CardList verbs={filteredVerbs} select={this.verbSelection} />
          </Scroll>
        )}
      </div>
    );
